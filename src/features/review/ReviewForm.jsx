...
export default function ReviewForm({ productId }) {
  ...
  return (
    <form onSubmit={submitHandler} className="space-y-4 mt-4 w-full max-w-full sm:max-w-lg">
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
        <label className="font-semibold min-w-[80px]">Rating:</label>
        <StarRatingInput rating={rating} setRating={setRating} />
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          onChange={(e) => setComment(e.target.value)}
          placeholder="Write your review..."
          className="border p-2 rounded w-full resize-none sm:text-sm"
          rows={4}
          required
        />
      </div>
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="bg-black text-white px-4 py-2 rounded hover:bg-gray-800 w-full sm:w-auto transition"
      >
        {isLoading ? "Submitting..." : "Submit Review"}
      </button>
    </form>
